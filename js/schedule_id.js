...
export default {
  ...
  methods: {
    calculateRepaymentSchedule() {
      ...
      this.$nextTick(() => {
        ...
        var totalInterest =
          parseInt(data.amount) * interestRate * parseInt(data.loan_term);
        this.ttInterest = totalInterest;
        this.ttBalance = parseInt(data.amount) + parseInt(totalInterest);
        var balance = parseInt(data.amount) + parseInt(totalInterest);
        for (var i = 0; i < data.number_of_repayments; i++) {
          ...
          item.paiddate = this.getNextWeekDay(
            (i + 1) *
              parseInt(this.getTermsValue(data.repaid_every_unit)) *
              parseInt(data.repaid_every)
          )
            .toLocaleString()
            .split(" ")[0];
          item.principaldue = data.amount / data.number_of_repayments;
    ...
    shareSchedule() {
      ...
      const user = this.$store.getters.getResults.filter(
        (user) => user?.username === username
      )[0];

      this.$store.dispatch("shareSchedule", {
        scheduleId: schedule?.id,
        shareTo: user?.username,
      });
  ...
  watch: {
    searchUsers(e) {
      this.$store.dispatch("_searchUsers", e);
    },
  },
  computed: {
    ...mapGetters({
      uLoading: "getUsersLoading",
    }),
    schedule() {
      var data = this.$store.getters.getterschedule(
        parseInt(this.$nuxt._route.params.id)
      );

      return data[0];
    },
    users() {
      return this.$store.getters.getResults.map((user) => user?.username);
    },
  },
};
